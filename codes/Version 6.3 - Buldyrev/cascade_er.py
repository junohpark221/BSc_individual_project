...
def analyse_attacked_network(net, coords, supporting_nodes, cas_data, step, killed_nodes):
...
def attack_network(net, coords, supporting_nodes, cas_data, attack_size):
    ...
    try:
        ...
        if attack_size > (len(coords) // len(net.get_layers())) or (attack_size <= 0):
            raise Exception("Wrong attack_size")

        initialise_state(coords)

        # With the different attack_type, sample the targets differently.
        targets = random.sample(list(range(len(coords) // len(attack_layer_list))), attack_size)
        ...
        cas_data = analyse_attacked_network(net, coords, supporting_nodes, cas_data, step, len(killed))
