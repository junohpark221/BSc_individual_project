...
def find_supporting_nodes(net, layer_names):
    if support_type == "random_nodes" or "random_layers":
        for cur_layer in range(len(layer_names)):
            ...
            for cur_node in cur_layer_nodes:
                supporting_nodes.append((cur_node, target_nodes[index]))
                index += 1
    else:
        for cur_node in range(len(coords)):
            supporting_node = -1
            cur_layer_index = int(cur_node // nodes)
            neighbours = list(net[cur_node, layer_names[cur_layer_index]])
            target_layers = []

            if cur_layer_index == 0:
                target_layers.append(layer_names[cur_layer_index + 1])
            elif cur_layer_index == (len(layer_names) - 1):
                target_layers.append(layer_names[cur_layer_index - 1])
            else:
                target_layers.append(layer_names[cur_layer_index - 1])
                target_layers.append(layer_names[cur_layer_index + 1])
    ...
    if cur_layer != 0:
        for i in range(cur_layer * nodes, (cur_layer + 1) * nodes):
            for j in range((cur_layer - 1) * nodes, cur_layer * nodes):
                ...
                if (j == supporting_nodes[i][1]) and (random.random() <= support_thres):
...
def build_network():
    ...
    find_supporting_nodes(net, layer_names)
...
if __name__ == "__main__":
    ...
    analyse_initial_network(network)
    ...
    attacked_network = cas.attack_network(network, coords, supporting_nodes, attack_type, attack_size=20, attack_layer='b', attack_point=(0.5, 0.5))
