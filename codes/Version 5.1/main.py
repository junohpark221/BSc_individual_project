    ...
    if (intra_type == 'RGG') and (inter_type == 'RGG'):
            ...
            for cur_node in range((cur_layer * nodes), (cur_layer + 1) * nodes):
                for target_node in range((cur_layer + 1) * nodes, (cur_layer + 2) * nodes):
                    ...
                    if d <= inter_thres:
                        ...
                        if cur_node == rgg_rgg_supp_nodes[target_node][1]:
                            for_supp_edges += 1
                        else:
                            for_edges += 1

                        if target_node == rgg_rgg_supp_nodes[cur_node][1]:
                            back_supp_edges += 1
                        else:
                            back_edges += 1
        ...
        if cur_layer != (len(layer_names) - 1):
            ...
            for target_node in target_nodes:
                if rgg_rand_supp_nodes[target_node][1] in cur_nodes:
                    net[rgg_rand_supp_nodes[target_node][1], target_node, layer_names[cur_layer], layer_names[cur_layer + 1]] = 1
                    inter_rand_edges.append((rgg_rand_supp_nodes[target_node][1], target_node, layer_names[cur_layer], layer_names[cur_layer + 1]))
                    for_supp_edges += 1
            ...
            for cur_node in cur_nodes:
                if rgg_rand_supp_nodes[cur_node][1] in target_nodes:
                    net[rgg_rand_supp_nodes[cur_node][1], cur_node, layer_names[cur_layer + 1], layer_names[cur_layer]] = 1
                    inter_rand_edges.append((rgg_rand_supp_nodes[cur_node][1], cur_node, layer_names[cur_layer + 1], layer_names[cur_layer]))
                    back_supp_edges += 1
    ...
    if (intra_type == 'RGG') and (inter_type == 'RGG'):
        for cur_layer in range(len(layer_names)):
            ...
            for cur_node in range(cur_layer * nodes, (cur_layer + 1) * nodes):
                ...
                rgg_rgg_supp_nodes.append((cur_node, short_node))
                if short_node != -1:
                    target_nodes.remove(short_node)
    elif (intra_type == 'Random') and (inter_type == 'RGG'):
        for cur_node, supp_node in rgg_rgg_supp_nodes:
            rand_rgg_supp_nodes.append((cur_node, supp_node))
    elif (intra_type == 'RGG') and (inter_type == 'Random'):
        for cur_layer in range(len(layer_names)):
            target_nodes = []
            if cur_layer == 0:
            ...
            for cur_node in cur_layer_nodes:
                rgg_rand_supp_nodes.append((cur_node, target_nodes[index]))
                index += 1
    elif (intra_type == 'Random') and (inter_type == 'Random'):
        for cur_node, supp_node in rgg_rand_supp_nodes:
            rand_rand_supp_nodes.append((cur_node, supp_node))
...
if __name__ == "__main__":
    ...
    att_rgg_rgg_net = cas.attack_network(rgg_rgg_net, coords, rgg_rgg_supp_nodes, attack_type, graph_type="RGG_RGG", attack_size=20, attack_point=(0.5, 0.5))
    ...
    att_rgg_rand_net = cas.attack_network(rgg_rand_net, coords, rgg_rand_supp_nodes, attack_type, graph_type="RGG_Rand", attack_size=20, attack_point=(0.5, 0.5))
    ...
    att_rand_rgg_net = cas.attack_network(rand_rgg_net, coords, rand_rgg_supp_nodes, attack_type, graph_type="Rand_RGG", attack_size=20, attack_point=(0.5, 0.5))
    ...
    att_rand_rand_net = cas.attack_network(rand_rand_net, coords, rand_rand_supp_nodes, attack_type, graph_type="Rand_Rand", attack_size=20, attack_point=(0.5, 0.5))
