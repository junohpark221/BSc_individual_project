    ...
    if (intra_type == 'RGG') and (inter_type == 'RGG'):
        if cur_layer != (len(layer_names) - 1):
            ...
            for cur_node in range((cur_layer * nodes), (cur_layer + 1) * nodes):
                if rgg_supp_nodes[cur_node] != -1:
                    net[cur_node, rgg_supp_nodes[cur_node], layer_names[cur_layer], layer_names[rgg_supp_nodes[cur_node] // nodes]] = 1
                    inter_rgg_edges.append((cur_node, rgg_supp_nodes[cur_node], layer_names[cur_layer], layer_names[rgg_supp_nodes[cur_node] // nodes]))

                    back_supp_edges += 1

            if cur_layer == (len(layer_names) - 2):
                for cur_node in range((cur_layer + 1) * nodes, (cur_layer + 2) * nodes):
                    if rgg_supp_nodes[cur_node] != -1:
                        net[cur_node, rgg_supp_nodes[cur_node], layer_names[cur_layer + 1], layer_names[rgg_supp_nodes[cur_node] // nodes]] = 1
                        inter_rgg_edges.append((cur_node, rgg_supp_nodes[cur_node], layer_names[cur_layer + 1], layer_names[rgg_supp_nodes[cur_node] // nodes]))
...
def make_data_frame(init_data, cas_data, rep, graph_type):
    ...
    """
    f = open('find intra thres.csv', 'a', newline='')
    wr = csv.writer(f)
