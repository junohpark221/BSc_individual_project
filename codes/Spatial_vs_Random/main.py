...
def make_interlayer_edges(net, cur_layer, layer_names, intra_type, inter_type):
    if (intra_type == 'RGG') and (inter_type == 'Spatial'):
        for_supp_edges = 0
        back_supp_edges = 0
...
def find_supporting_nodes(intra_type, inter_type):
...
def build_network(intra_type, inter_type):
    ...
    find_supporting_nodes(intra_type, inter_type)
...
if __name__ == "__main__":
    ...
    """
        ...
        For "spatial_range" attack,         cas.attack_network(network, coords, supporting_nodes, attack_type, attack_point=(0.5, 0.5), attack_radius=0.1)
    ...
    for i in range(500):
        ...
        rgg_rgg_net = build_network(intra_type='RGG', inter_type='Spatial')
        init_data = analyse_initial_network(rgg_rgg_net, init_data)
        # draw_network(rgg_rgg_net, type="RGG_Spatial")
        att_rgg_rgg_net, cas_data = cas.attack_network(rgg_rgg_net, coords, rgg_supp_nodes, cas_data, attack_type, attack_size, attack_point=attack_point)
        make_data_frame(init_data, cas_data, rep, graph_type='RGG_Spatial')
        ...
        # draw_network(rgg_rand_net, type="RGG_Random")
        att_rgg_rand_net, cas_data = cas.attack_network(rgg_rand_net, coords, rand_supp_nodes, cas_data, attack_type, attack_size, attack_point=attack_point)
        ...
        rand_rgg_net = build_network(intra_type='ER', inter_type='Spatial')
        init_data = analyse_initial_network(rand_rgg_net, init_data)
        # draw_network(rand_rgg_net, type="ER_Spatial")
        att_rand_rgg_net, cas_data = cas.attack_network(rand_rgg_net, coords, rgg_supp_nodes, cas_data, attack_type, attack_size, attack_point=attack_point)
        make_data_frame(init_data, cas_data, rep, graph_type='ER_Spatial')
        ...
        rand_rand_net = build_network(intra_type='ER', inter_type='Random')
        init_data = analyse_initial_network(rand_rand_net, init_data)
        # draw_network(rand_rand_net, type="ER_Random")
        att_rand_rand_net, cas_data = cas.attack_network(rand_rand_net, coords, rand_supp_nodes, cas_data, attack_type, attack_size, attack_point=attack_point)
        make_data_frame(init_data, cas_data, rep, graph_type='ER_Random')
    ...
    # draw_network(att_rgg_rgg_net, type="Attacked RGG_Spatial")
    # draw_network(att_rgg_rand_net, type="Attacked RGG_Random")
    # draw_network(att_rand_rgg_net, type="Attacked ER_Spatial")
    # draw_network(att_rand_rand_net, type="Attacked ER_Random")
