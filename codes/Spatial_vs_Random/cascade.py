...
def supported(net, coords, supporting_nodes, layer_names, cur_node):
    is_supported = False
    supp_node = supporting_nodes[cur_node]
...
def check_far_dead_node(coords, killed, attack_point):
    temp_far_dist = 0
    for target in killed:
        d = cal_dist(attack_point, target, coords)
        if d > temp_far_dist:
            temp_far_dist = d

    far_dead_node.append(temp_far_dist)
...
def find_near_nodes(net, coords, attack_size, attack_point, attack_layer_list):
    ...
    for cur_layer in range(len(attack_layer_list)):
        for cur_node in range(len(coords)//len(net.get_layers())):
...
def analyse_attacked_network(net, coords, cas_data, step, attack_point):
...
def attack_network(net, coords, supporting_nodes, cas_data, attack_type, attack_size=5, attack_point=(0.5, 0.5), attack_radius=0.1):
    ...
    try:
        ...
        while len(killed) != 0:
            ...
            check_far_dead_node(coords, killed, attack_point)
        ...
        cas_data = analyse_attacked_network(net, coords, cas_data, step, attack_point)
