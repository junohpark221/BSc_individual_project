...
def supported(net, coords, supporting_nodes, layer_names, cur_node):
    is_supported = True
    supportings = supporting_nodes[cur_node]

    cur_layer = int(cur_node // (len(coords)//len(layer_names)))

    num = 0
    for cur_supp in supportings:
        supporting_layer = int(cur_supp // (len(coords) // len(layer_names)))
        if net[cur_supp, cur_node, layer_names[supporting_layer], layer_names[cur_layer]] == 0:
            num += 1

    if num >= len(supportings):
        is_supported = False
...
def attack_network(net, coords, supporting_nodes, attack_type, graph_type="RGG_RGG", attack_size=5, attack_point=(0.5, 0.5), attack_radius=0.1):
