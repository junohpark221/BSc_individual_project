...
def find_relative_pos(x1, x2, y1, y2):
    if x1 <= x2:
        left = (x1, y1)
        right = (x2, y2)
    else:
...
def cal_dist(cur_node, target_node):
    ...
    left, right, low, high = find_relative_pos(x1, x2, y1, y2)

    dists.append(math.sqrt((x1 - x2)**2 + (y1 - y2)**2))
    dists.append(math.sqrt((low[0] - high[0])**2 + (low[1] + 1 - high[1])**2))
    dists.append(math.sqrt((left[0] + 1 - right[0])**2) + (left[1] + 1 - right[1])**2)
    dists.append(math.sqrt((left[0] + 1 - right[0])**2) + (left[1] - right[1])**2)
    dists.append(math.sqrt((left[0] + 1 - right[0])**2) + (left[1] - 1 - right[1])**2)
...
if __name__ == "__main__":
    ...
    attacked_network = cas.attack_network(network, coords, supporting_nodes, attack_type, attack_size=20, attack_layer='a', attack_point=(0.5, 0.5))
